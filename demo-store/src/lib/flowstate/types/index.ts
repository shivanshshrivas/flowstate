...
// Ordered states for progress bar (excluding DISPUTED which is a branch)
...
export interface EscrowDetails {
  ...
  tokenAddress: string;
  totalAmount: string; // in token units (BigInt string)
...
export interface Order {
  ...
  state: OrderState;
  total_usd: number;
  total_token: string; // MockRLUSD amount (BigInt string)
...
export interface PayoutConfig {
  immediate_bps: number; // paid at label printed
  milestone_bps: number; // paid across shipping states
  holdback_bps: number; // paid at finalization
  // must sum to 10000
}
...
export interface SellerMetrics {
  ...
  dispute_rate: number; // 0-1 percentage
...
export interface User {
  ...
  role: UserRole;
  wallet_address?: string;
  seller_id?: string; // if role === 'seller'
}
